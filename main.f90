program main
    ...
    character(100), parameter ::    mesh        = 'gmsh/monai.su2'  ! mesh file
    ...
    integer :: it
    real(precision) :: t
    real(precision), dimension(:), allocatable :: ht
...
contains
    ...
    subroutine set_initial()
        ...
        !     if (sqrt((ccx - 25.)**2 + (ccy - 25.)**2) <= 10.) then
        !         h(i) = 2.
        !     end if
        ! end do
        ...
        ! integer :: i
        ! real(precision) :: gradient, ccx, ccy
        ...
        !     if (4. <= ccx .and. ccx <= 4.45) then
        !         b(i) = gradient*(ccx - 4.)
        !     end if

        !     if (4.45 <= ccx .and. ccx <= 4.9) then
        !         b(i) = -gradient*(ccx - 4.45) + .065
        !         h(i) = max(dry, .02 - b(i))
        !     end if

        !     if (0. <= ccx .and. ccx <= 2.39) then
        !         h(i) = .111
        !     end if

        !     if (4.9 <= ccx .and. ccx <= 5.6) then
        !         h(i) = .02
        !     end if
        ! end do

        ! Case Study 3 : Monai Valley
        ! [O] Inflow Boundary
        ! [X] Outflow Boundary
        ! [O] Solid Boundary
        ! [O] Slope Source
        ...
        integer :: io, i, ht_size
        real(precision) :: b_value, ht_value
        ...
        open(10, file=trim(data)//'b.txt', action='read')
        
        do i = 1, n_cells
            read(10, *, iostat=io) b_value
            b(i) = b_value
            h(i) = max(dry, .135 - b_value)
        end do

        open(10, file=trim(data)//'ht.txt', action='read')

        ht_size = ceiling(tt/dt)
        allocate(ht(ht_size))
        do i = 1, ht_size
            read(10, *, iostat=io) ht_value
            ht(i) = ht_value
        end do
    ...
    subroutine set_convection()
        ...
        do i = 1, n_faces
            ...
            if (any(i == solid_faces)) then  ! solid face
                F(i, 1) = 0.
                F(i, 2) = g*hLj**2*nfx/2.
                F(i, 3) = g*hLj**2*nfy/2.
            else
                ...
                if (any(i == inflow_faces)) then  ! inflow face
                    hRj = ht(it)  ! specified depth
                    uhR = uhL + 2.*sqrt(g*hLj) - 2*sqrt(g*hRj)
                    uR = uhR*nfx - vhL*nfy
                    vR = vhL*nfx + uhR*nfy
                    vhR = -uR*nfy + vR*nfx
                    qxRj = hRj*uR
                    qyRj = hRj*vR
                else  ! outflow/interior face
                    hRj = hL(cj, fj)
                    qxRj = qxL(cj, fj)
                    qyRj = qyL(cj, fj)
    ...
    subroutine set_data()
        ...
        if (mod(it - 1, interval) == 0) then
            write(file, '(a, i10.10, a)') 'z', it - 1, '.txt'
